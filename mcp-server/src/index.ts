...
import { runScript, type BridgeOptions } from "./bridge.js";
...
const server = new Server(
  { name: "wiki-context", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "wiki_query":
        return await handleWikiQuery(
          opts,
          { workspace: String(i.workspace ?? opts.workspace), q: String(i.q ?? ""), k: Number(i.k ?? 3) },
          runScript
        );
      case "wiki_ingest":
        return await handleWikiIngest(
          opts,
          { workspace: String(i.workspace ?? opts.workspace), pages: String(i.pages ?? ""), project: i.project as string | undefined },
          runScript
        );
      case "wiki_lint":
        return await handleWikiLint(
          opts,
          { workspace: String(i.workspace ?? opts.workspace), project: i.project as string | undefined },
          runScript
        );
      case "wiki_serve":
    ...
    return { content: [{ type: "text" as const, text: `Errore: ${String(err)}` }], isError: true };
  }
});
