...
if (isMain) {
    ...
    if (isUninstall) {
        ...
        if (settings.hooks?.UserPromptSubmit) {
            settings.hooks.UserPromptSubmit = settings.hooks.UserPromptSubmit.filter((h) => !h.hooks.some((c) => c.command.includes("wiki_context.py")));
        }
        if (!isDryRun)
    ...
    const existingHookIdx = settings.hooks.UserPromptSubmit.findIndex((h) => h.hooks.some((c) => c.command.includes("wiki_context.py")));
